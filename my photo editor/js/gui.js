...
function GUI_CLASS() {
	...
	//modify: hide grid when start: add new functin in file 
	//document.readyState = pre_loader();

	function pre_loader() {
		if (FILE.check_open) {
			document.getElementById('canvas_back').style.display = 'none';
			
		}
		else {
			document.getElementById('canvas_back').style.display = 'auto';
	...
	this.draw_helpers = function () {
		...
		for (var i in DRAW_TOOLS_CONFIG) {
			...
			html += ' style="background:' + draw_colors[i] +';"';
